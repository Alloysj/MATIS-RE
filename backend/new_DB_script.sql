-- 1. Enumerated types (enums) definitions
...
CREATE TABLE public.transactions (
  ...
  type character varying NOT NULL,  -- you didn’t specify an enum for “type”, so using varchar
...
CREATE TABLE public.payment_allocations (
  ...
  category character varying NOT NULL,  -- assuming category not enum; adjust if needed
  amount numeric NOT NULL,
...
CREATE TABLE public.loans (
  ...
  vehicle_id uuid,
  type character varying NOT NULL,  -- you didn’t define enum for type; adjust if needed
  amount numeric NOT NULL,
  purpose text,
...
CREATE TABLE public.salary_advances (
  ...
  reason text,
  application_date timestamp with time zone NOT NULL DEFAULT now(),
  status salary_advance_status NOT NULL DEFAULT 'PENDING',
  approved_by uuid,
  approved_at timestamp with time zone,
...
ALTER SEQUENCE public.users_member_number_seq
  OWNED BY public.users.member_number;
